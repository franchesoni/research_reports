...
available_datasets = [
    ...
    "Damage Detection of Power Plants",
    "Cocoa Diseases",
    "Rice Disease",
    "WGISD",
    "Wind Turbine Detection (by Luke Borkowski)",
    "Cityscapes",
    "Maize Cobs",
    "Pylon Components",
    "STN PLAD",
    ...
    "Wind Turbines (by Kyle Graupe)",
    "California and Arizona Wind Turbines (by Duke Dataplus2020)",
    "Overhead Imagery of Wind Turbines (by Duke Dataplus2020)",
    "Large Wind Turbines (by Duke Dataplus2020)",
    "Wind Turbine Detection (by Saurabh Shahane)",
    "Multi-topography Dataset for Wind Turbine Detection",
    "YOLO Annotated Wind Turbine Surface Damage",
    "Wind Turbine Detection (by Noah Vriese)",
    "Self Driving Cars",
    "SARAS-ESAD 2020",
    "BCCD",
    ...
    "Heat Sink Surface Defect",
    "Bee Image Object Detection",
    "UAV Small Object Detection (UAVOD-10)",
    ...
    "Fabric Stain",
    "Road Pothole Images",
    "Aerial Power Infrastructure",
    "Detection of Small Size Construction Tools",
    "Piling Sheet Image Data",
    "MVTec AD",
    "MVTec D2S",
    "QuinceSet",
    "MVTec LOCO AD",
    "FloodNet 2021 (Track 1)",
    "Supervisely Synthetic Crack Segmentation",
    "Concrete Crack Segmentation",
    "CNAM-CD",
    "Photometric Stereo Leafs",
    "CCAgT",
    "WIDER FACE",
    "OpenCow2020",
    "UAVid",
    "Automatic Monitoring of Pigs",
    "Labeled Surgical Tools and Images",
    "xView 2018",
    "Fruit Recognition",
    "Intraretinal Cystoid Fluid",
    "Severstal",
    "Maize Whole Plant Image",
    "COCO-Stuff 10k",
    "AeroScapes",
    "EDD2020",
    "PCBSegClassNet",
    ...
    "GWHD 2021",
    "Cattle Detection and Counting in UAV Images",
    "CBIS-DDSM",
    ...
    "DOTA",
    "iSAID",
    "Surgical Scene Segmentation in Robotic Gastrectomy",
    "PACO-LVIS",
    "Indian Roads",
    "CitySegmentation",
    "Pothole Detection",
    ...
    "HIT-UAV",
    "Strawberry Dataset for Object Detection",
    "Urban Street: Flower Classification",
    "Urban Street: Fruit Classification",
    "Urban Street: Tree Classification",
    "Urban Street: Leaf Classification",
    "Urban Street: Branch",
    "Urban Street: Leaf",
    "Urban Street: Trunk",
    "Urban Street: Tree",
    "ABU Robocon 2021 Pot",
    "Vehicle Dataset for YOLO",
    "SISVSE",
    "Outdoor Hazard Detection",
    "HyperKvasir Images",
    "OD-WeaponDetection: Knife Classification",
    "OD-WeaponDetection: Pistol Classification",
    "OD-WeaponDetection: Knife Detection",
    "OD-WeaponDetection: Pistol Detection",
    "OD-WeaponDetection: Sohas Classification",
    "OD-WeaponDetection: Sohas Detection",
    "AFID",
    ...
    "Intruder Detection",
    "Guns in an Active State Detection",
    "Gesture v1.0",
    "BLPR: License Plate Localization",
    "BLPR: Character Recognition",
    "Carrot-Weed",
    "CADI-AI",
...
class NinjaDataset:
    def __init__(self, ds_path, split="train", flip_mode=False, special_ds_dir=None):
        ...
        if train_dir and test_dir:
            ...
            if split.lower() == "train":
                self.img_list = [
                    os.path.join(ds_path, ds_dir, "img", all_imgs[i])
                    for i in indices[:split_ratio]
                ]
                self.ann_list = [
                    os.path.join(ds_path, ds_dir, "ann", all_anns[i])
                    for i in indices[:split_ratio]
                ]
            ...
            raise ValueError(
                "Dataset must have either train and test splits or a ds folder"
            )
    ...
    def __getitem__(self, idx):
        ...
        for mind, obj in enumerate(objects):
            ...
            if (
                origin[1] + seg.shape[0] > img.height
                or origin[0] + seg.shape[1] > img.width
            ):
                rotated = True
        if rotated:
            for seg, origin in zip(segs, origins):
                assert (
                    origin[1] + seg.shape[0] <= img.width
                    or origin[0] + seg.shape[1] <= img.height
                )
        ...
        if self.flip_mode == "central":
            masks = masks[
                :, ::-1, ::-1
            ]  # flip masks vertically and horizontally, for some reason they are flipped in the dataset
...
def describe_sample(image, masks, class_names):
    ...
    description["canny_per"] = (
        cv2.Canny(np.array(image), 100, 200) > 0
    ).sum() / description["image_size"]
    ...
    description["area_perimeter_ratio"] = [
        regprop.area / regprop.perimeter for regprop in regprops
    ]
    return description
...
def describe_dataset(sample_descriptions, mask_avg):
    ...
    n_instances_per_image = sum(
        [sample_desc["n_instances"] for sample_desc in sample_descriptions]
    ) / len(sample_descriptions)
    # get the percentage of empty masks
    empty_masks_per = sum(
        [sample_desc["empty_mask"] for sample_desc in sample_descriptions]
    ) / len(sample_descriptions)

    max_instances, min_instances = max(
        [sample_desc["n_instances"] for sample_desc in sample_descriptions]
    ), min([sample_desc["n_instances"] for sample_desc in sample_descriptions])
        ...
        for sample_desc in sample_descriptions:  # dicts with keys and lists as values
            for ind in range(len(sample_desc["class_names"])):
                if sample_desc["empty_mask"]:
                    continue
                new_entry = {
                    key: sample_desc[key][ind]
                    if isinstance(sample_desc[key], list)
                    else sample_desc[key]
                    for key in sample_desc.keys()
                }
    ...
    image_size_mean = np.array(
        [sample_desc["image_size"] for sample_desc in sample_descriptions]
    ).mean()
    image_size_std = np.array(
        [sample_desc["image_size"] for sample_desc in sample_descriptions]
    ).std()
    # get the average canny percentage
    canny_per_mean = np.array(
        [sample_desc["canny_per"] for sample_desc in sample_descriptions]
    ).mean()
    canny_per_std = np.array(
        [sample_desc["canny_per"] for sample_desc in sample_descriptions]
    ).std()
    ...
    spatial_unpredictability = (-(mask_avg * np.log(mask_avg + 1e-8)).sum()) / (
        mask_avg.shape[0] * mask_avg.shape[1]
    )
    # print the results
    retdict = {
...
def download_if_needed(datasets_dir, dataset):
    # get dst dir
    ...
    return ds_path, ds_nickname


def load_dataset(datasets_dir, dataset):
    ds_path, ds_nickname = download_if_needed(datasets_dir, dataset)
    # load the dataset
    flip_mode = "central" if dataset in ("Carrot-Weed") else False
    special_ds_dir = (
        "segmentation1"
        if dataset == "Panoramic Dental X-rays"
        else "synthetic cracks"
        if dataset == "Supervisely Synthetic Crack Segmentation"
        else None
    )
    ds = NinjaDataset(ds_path, flip_mode=flip_mode, special_ds_dir=special_ds_dir)
    return ds, ds_path, ds_nickname


def prepare_and_visualize_all(datasets_dir, n_samples_to_visualize, dstpath='gitignored'):

    for dataset in benchmark_datasets:
        ds, ds_path, ds_nickname = load_dataset(datasets_dir, dataset)
        # reset and create dst dir
        dstdir = Path(dstpath) / ds_nickname
        shutil.rmtree(dstdir, ignore_errors=True)
        dstdir.mkdir(parents=True)

        for ind, sample in tqdm.tqdm(
            enumerate(ds), total=n_samples_to_visualize
        ):  # for the first few samples save the images and masks
            if ind == n_samples_to_visualize:
                break
            img, masks, class_names = sample
            if (
                masks.shape[0] == 0
            ):  # if masks is empty, check for all the dataset how many empty masks do we have
                with_mask, wo_mask = [], []
                count = 0
                for i, (img, masks, class_names) in tqdm.tqdm(
                    enumerate(ds), total=len(ds)
                ):
                    if masks.shape[0] > 0:
                        with_mask.append(i)
                        if count < n_samples_to_visualize:
                            # save images and first N masks
                            img.save(dstdir / f"img_{i}.png")
                            for mind, mask in enumerate(masks[:n_samples_to_visualize]):
                                Image.fromarray(mask).save(
                                    dstdir / f"mask_{i}_{mind}_{class_names[mind]}.png"
                                )
                            count += 1
                    else:
                        wo_mask.append(i)
                print("!" * 20)
                print(
                    f"{dataset} has {len(with_mask)} images with masks and {len(wo_mask)} without masks"
                )
                print("!" * 20)
                break
            # save images and first N masks
            ...
            for mind, mask in enumerate(masks[:n_samples_to_visualize]):
                Image.fromarray(mask).save(
                    dstdir / f"mask_{ind}_{mind}_{class_names[mind]}.png"
                )


def describe_all_datasets(datasets_dir, dstpath='gitignored'):
    for dataset in benchmark_datasets:
        # get dst dir
        ds_nickname = dataset.lower().replace(" ", "-")
        ds_path = os.path.join(datasets_dir, ds_nickname)
        ...
        for ind, sample in tqdm.tqdm(enumerate(ds), total=len(ds)):
            ...
            avg_mask_num = avg_mask_num + skimage.transform.resize(
                (masks * 1).sum(axis=0), (512, 512)
            )
        ...
        np.save(Path(dstpath) / f"{ds_nickname}_desc.npy", ds_desc)


if __name__ == "__main__":
    datasets_dir = "/home/franchesoni/adisk/ninjads/"
    n_images_to_visualize = 5
    prepare_and_visualize_all(datasets_dir, n_images_to_visualize, dstpath='gitignored')
    describe_all_datasets(datasets_dir=datasets_dir)
